using Plots, SpecialFunctions, Distributions, HCubature, QuadGK
...
h(x::Float64) = exp(-x^4)
g(x::Float64) = exp(-x^2/2) / sqrt(2π)
fg(x::Float64) = exp(-x^2/2 - x^4) / sqrt(2π)
...
# C, err = quadgk(f_n, -300, 300)
a = 1 / C
err_C = (1/C)^2 * err
round.(a, digits = 12)
round.(err_C , digits = 12)

# acc = quadgk(fg, -10, 10, rtol= 1e-3)
sol  = hquadrature(fg, -300, 300)
acc, err = round.(sol, digits = 12)
C_acc = 1 / (acc * sqrt(2π))
err_acp = (1/acc)^2 * err / sqrt(2π)
round.(C_acc, digits = 12)
